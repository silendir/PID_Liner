...
- (NSArray<CrashAnomalyIndicator *> *)localDetectAnomalies:(PIDCSVData *)data {
    ...
    // 7. 电机缺相（某轴陀螺仪持续偏移）
    CrashAnomalyIndicator *phaseInd = [self detectPhaseLoss:gyroArrays];
    // 缺相用整个陀螺仪数据的时间线
    phaseInd.firstOccurrenceRatio = 0;
    phaseInd.peakOccurrenceRatio = 0.5;
    phaseInd.lastOccurrenceRatio = 1.0;
...
- (CrashAnomalyIndicator *)detectPhaseLoss:(NSArray<NSArray<NSNumber *> *> *)gyroArrays {
    ...
    for (NSInteger axis = 0; axis < 3; axis++) {
        ...
        if (gyro.count == 0) continue;

        // 计算均值偏移
        double sum = 0;
        for (NSNumber *v in gyro) sum += v.doubleValue;
        double mean = sum / gyro.count;

        // 计算有多少比例的点偏向同一方向
        NSInteger positiveCount = 0;
        for (NSNumber *v in gyro) {
            if (v.doubleValue > 0) positiveCount++;
        }
        double biasRatio = fabs((double)positiveCount / gyro.count - 0.5) * 2; // 0~1, 越大越偏

        if (biasRatio > worstBias) {
            worstBias = biasRatio;
            worstAxis = axisNames[axis];
        }
    }

    ind.detected = (worstBias > 0.3);
    ind.severity = worstBias > 0.5 ? @"high" : @"medium";
    ind.axis = worstAxis;
    ind.detail = [NSString stringWithFormat:@"最大轴偏移比: %.2f (%@轴)", worstBias, worstAxis ?: @"无"];
...
- (NSDictionary *)buildAIRequest:(NSDictionary *)featureSummary {
    ...
    // 🔑 极简 prompt：角色+术语+格式 合并为一段
    [sp appendString:@"你是FPV老飞手。用中文诊断,按时间线叙事,以「真相只有一个!」总结根因。"
        ...
        @"不提算法/AI。root_cause先发生,caused_by是结果。正常就说未检测到异常。\n"];
...
- (void)streamToZhipuAI:(NSDictionary *)body
    ...
    // 创建带 delegate 的 session（主线程回调，方便更新 UI）
...
- (void)processSSEMessage:(NSString *)message {
    for (NSString *line in [message componentsSeparatedByString:@"\n"]) {
        ...
        // 🔑 流式回调：只展示 content（最终输出），永远不暴露 reasoning_content 给用户
        if (self.contentBuffer.length > 0 && self.onStreamingText) {
            self.onStreamingText([self.contentBuffer copy]);
